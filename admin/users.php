...
if ($action === 'delete' && $userId) {
    try {
        ...
        header('Location: /admin/users.php' . ($userType ? "?type=$userType" : ''));
        exit();
...
if ($action === 'verify' && $userId) {
    try {
        ...
        header('Location: /admin/users.php?type=provider');
        exit();
    } catch (PDOException $e) {
        $errors['database'] = 'Database error: ' . $e->getMessage();
    }
...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div>
                <div class="btn-group">
                    <a href="/admin/users.php" class="btn btn-outline-primary <?php echo $userType === '' ? 'active' : ''; ?>">All Users</a>
                    <a href="/admin/users.php?type=customer" class="btn btn-outline-primary <?php echo $userType === 'customer' ? 'active' : ''; ?>">Customers</a>
                    <a href="/admin/users.php?type=provider" class="btn btn-outline-primary <?php echo $userType === 'provider' ? 'active' : ''; ?>">Providers</a>
                    <a href="/admin/users.php?type=admin" class="btn btn-outline-primary <?php echo $userType === 'admin' ? 'active' : ''; ?>">Admins</a>
                </div>
            </div>
        </div>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <?php if ($user['user_type'] === 'provider' && $user['has_profile'] && !$user['is_verified']): ?>
                                                    <a href="/admin/users.php?action=verify&id=<?php echo $user['id']; ?>" class="btn btn-success" onclick="return confirm('Are you sure you want to verify this provider?')">
                                                        <i class="fas fa-check"></i> Verify
                                                    </a>
                                                <?php endif; ?>
                                                
                                                <?php if ($user['user_type'] !== 'admin'): ?>
                                                    <a href="/admin/users.php?action=delete&id=<?php echo $user['id']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this user? This action cannot be undone.')">
